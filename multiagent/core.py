...
class World(object):
    def __init__(self):
        ...
        # Fixed-wing mode (optional; enabled by orbit_graph scenario)
        self.fixed_wing = False
        self.v_star = None      # constant speed (float)
        self.a_n_max = None     # clamp for normal accel (float or None)
        self.u_to_an_gain = 1.0  # gain in fixed-wing mapping: a_n = u_to_an_gain * v_star^2 * (u dot N)
    ...
    def step(self):
        ...
        for agent in self.agents:
            agent.state.p_pos_prev = np.array(agent.state.p_pos, dtype=np.float32)
    ...
    def integrate_state(self, p_force):
        # -----------------------------
        # Fixed-wing constant-speed branch
        # -----------------------------
        if getattr(self, "fixed_wing", False):
            v_star = float(self.v_star) if self.v_star is not None else None
            a_n_max = self.a_n_max if hasattr(self, "a_n_max") else None

            for i, entity in enumerate(self.entities):
                if not entity.movable:
                    continue
                # Only apply fixed-wing dynamics to agents
                if entity not in self.agents:
                    continue

                # u is the auxiliary vector field input in R^2 (from policy)
                u = entity.action.u if entity.action.u is not None else np.zeros(self.dim_p, dtype=np.float32)
                u = np.asarray(u, dtype=np.float32)

                # current velocity; if degenerate, initialize along +x
                v = np.asarray(entity.state.p_vel, dtype=np.float32)
                vnorm = np.linalg.norm(v)
                if vnorm < 1e-8:
                    if v_star is None:
                        v_star_local = 1.0
                    else:
                        v_star_local = max(1e-6, v_star)
                    v = np.array([v_star_local, 0.0], dtype=np.float32)
                    vnorm = v_star_local

                # enforce constant speed
                if v_star is None:
                    v_star_local = float(vnorm)
                else:
                    v_star_local = max(1e-6, float(v_star))

                T = v / max(1e-8, float(vnorm))
                N = np.array([-T[1], T[0]], dtype=np.float32)  # body normal

                # normal acceleration command
                u_gain = float(getattr(self, "u_to_an_gain", 1.0))
                a_n = u_gain * (v_star_local ** 2) * float(np.dot(u, N))
                if a_n_max is not None:
                    a_n = float(np.clip(a_n, -abs(a_n_max), abs(a_n_max)))

                # heading rate and discrete rotation
                dpsi = (a_n / v_star_local) * self.dt
                c = float(np.cos(dpsi))
                s = float(np.sin(dpsi))
                ...
                if v_next_norm < 1e-8:
                    v_next = np.array([v_star_local, 0.0], dtype=np.float32)
                    v_next_norm = v_star_local

                v_next = (v_star_local / v_next_norm) * v_next

                p_next = np.asarray(entity.state.p_pos, dtype=np.float32) + v_next * self.dt

                # disturbance (same shape as before: [pos(2), vel(2)])
                if entity in self.agents:
                    w = entity.disturbance.w
                    if w is not None:
                        w = np.asarray(w, dtype=np.float32)
                        p_next = p_next + w[: self.dim_p]
                        v_next = v_next + w[self.dim_p :]
                        # renormalize after disturbance to maintain constant speed
                        vn = np.linalg.norm(v_next)
                        if vn < 1e-8:
                            v_next = np.array([v_star_local, 0.0], dtype=np.float32)
                        else:
                            v_next = (v_star_local / vn) * v_next

                entity.state.p_pos = p_next
                entity.state.p_vel = v_next

            return  # IMPORTANT: skip default integrator

        # -----------------------------
        # Default MPE integrator (unchanged)
        # -----------------------------
        for i, entity in enumerate(self.entities):
            if not entity.movable:
                continue
