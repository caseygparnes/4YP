...
class MultiAgentGraphEnv(MultiAgentBaseEnv):
    ...
    def step(self, action_n: List) -> Tuple[List, List, List, List, List, List, List]:
        ...
        # ---- Ensure shapes match OnPolicy buffer expectations ----

        reward_n = np.asarray(reward_n, dtype=np.float32)
        reward_n = np.squeeze(reward_n)
        reward_n = reward_n.reshape(self.n, 1)
    ...
    def _get_disturbance_graph(self, agent: Agent):
        ...
        # Create disturbance graph with same shape as node_obs
        # IMPORTANT: keep it truly zero when w==0 (do NOT copy entity_type or goal features)
        disturbance_graph = np.zeros((num_entities, node_feature_dim), dtype=np.float32)

        # Disturbance vector convention in core.py integrator:
        # w = [pos_dx, pos_dy, vel_dx, vel_dy]
        disturbance_dim = 2 * self.world.dim_p  # 4 for 2D
        ...
        for i, entity in enumerate(self.world.entities):
            ...
            if w is None:
                w = np.zeros(disturbance_dim, dtype=np.float32)
            else:
                w = np.asarray(w, dtype=np.float32)

            # Node feature convention in orbit_graph is:
            # node_feat = [vel(2), pos(2), goal/center(2), entity_type(1)]
            # So map disturbance correctly:
            disturbance_graph[i, 0:self.world.dim_p] = w[self.world.dim_p:]              # vel disturbance
            disturbance_graph[i, self.world.dim_p:2*self.world.dim_p] = w[:self.world.dim_p]  # pos disturbance
    ...
    def reset(self) -> Tuple[List, List, List, List]:
        ...
        for agent in self.agents:
            ...
            # Set disturbance at t=0 so reset returns the same disturbance format as step()
            self._set_disturbance(agent)
            disturbance_n.append(self._get_disturbance_graph(agent))
