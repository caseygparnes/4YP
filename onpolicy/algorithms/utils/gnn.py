...
class StableConv(MessagePassing):
    ...
    def __init__(self,
        ...
        # First linear layer (NO BIAS)
        # NOTE: We don't add edge_dim because edge attributes are ignored in message()
        # to preserve zero-preservation property
        self.lin1 = nn.Linear(input_dim, hidden_size, bias=False)

        # Hidden layers (NO BIAS, NO LayerNorm)
        self.layers = nn.ModuleList()
        for _ in range(layer_N):
    ...
    def message(self, x_j: Tensor, edge_attr: OptTensor):
        """
        Process messages WITHOUT entity embeddings.
        x_j contains only node features (no entity type).

        CRITICAL: Ignore edge_attr to preserve zero-preservation property!
        Edge distances aren't part of the dynamical system state and would
        break f(0)=0 since distances are always non-zero.
        """
...
class StableTransformerConvNet(nn.Module):
    ...
    def __init__(self,
        ...
        # First transformer conv layer (NO BIAS)
        ...
        self._disable_transformer_biases(self.gnn1)
    ...
    def _disable_transformer_biases(self, transformer_conv):
        """
        Manually disable all biases in TransformerConv layer.

        PyTorch Geometric's TransformerConv has internal linear layers
        (lin_key, lin_query, lin_value, lin_skip) that have biases even
        when bias=False is set. We need to manually set these to None
        to ensure true zero-preservation for L_p-stability.
        """
        for attr_name in ['lin_key', 'lin_query', 'lin_value', 'lin_skip']:
            if hasattr(transformer_conv, attr_name):
                linear_layer = getattr(transformer_conv, attr_name)
                if hasattr(linear_layer, 'bias') and linear_layer.bias is not None:
                    # Delete the bias parameter
                    linear_layer.bias = None
                    linear_layer.register_parameter('bias', None)
...
class StableGNNBase(nn.Module):
    ...
    def forward(self, node_obs: Tensor, adj: Tensor, agent_id: Tensor):
        ...
        batch = torch.arange(batch_size, device=node_obs.device).repeat_interleave(num_nodes)
        # Create PyG Data object
        data = Data(x=x, edge_index=edge_index, edge_attr=None, batch=batch)        # IMPORTANT: edge_attr=None to preserve Lp stability
